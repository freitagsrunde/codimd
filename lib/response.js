...
exports.findNotesWithPermission = findNotesWithPermission
...
function findNotesWithPermission (req, res, next) {
  let invalidNotes = ['favicon.ico', 'status', 'build', 'robots.txt']

  models.Note.findAll(
    {attributes: ['id', 'shortid', 'alias', 'permission', 'title', 'content', 'authorship', 'createdAt', 'updatedAt', 'ownerId', 'lastchangeuserId']}
  ).then((notes) => {
    let parsedNotes = notes.filter(note => checkViewPermission(req, note))
      .map(note => {
        return {
          id: note.alias || note.shortid,
